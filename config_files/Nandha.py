...
while True:
    ...
    if len(classIds) > 0:
        # Convert bbox and confs to lists and map confs to floats
        bbox = list(bbox)
        ...
        for i in indicies.flatten():
            ...
            # Access the class name correctly
            classId = int(classIds[i])  # Flatten classId access
            label = classNames[classId - 1]  # Get the corresponding label
